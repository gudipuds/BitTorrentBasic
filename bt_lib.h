...
/*Length of fixed strings in info data construction*/
#define FIXED_STR_LENGTH 32

/*Hand shake message length*/
#define HAND_SHAKE_MSG_LENGTH 256
...
typedef struct {
  ...
  int leecher; //to check if the peer is running as a seeder or leecher
...
/* Returns the data from the socket message*/ 
void getDataFromSockMsg(char * sockMsg, char * sockData, unsigned  int length);

/* Returns the length of data from the socket message*/
int getLengthFromSockMsg(char * sockMsg);

/* Prints the progress of the downloaded file */
