...
void parse_args(bt_args_t * bt_args, int argc,  char * argv[]){
  ...
  bt_args->leecher = 0; //Considering the peer to be seeder initially
  ...
  strncpy(bt_args->save_file,"Downloads",FILE_NAME_MAX);
  ...
  while ((ch = getopt(argc, argv, "hb:p:s:l:vI:")) != -1) {
    ...
    case 'l': //log file
      ...
      break;
	case 'b': //Binding ip address and port number
	  bt_args->bindPeer = (peer_t *)malloc(sizeof(peer_t));
	  __parse_peer(bt_args->bindPeer, optarg);
	  break;
    case 'p': //peer
	  bt_args->leecher = 1;
     // n_peers++;
